<?php
	...
	require_once('php/crud_unidade.php');
  ...
  if(!isset($_SESSION)) {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['unidade_id'] = $_GET['id'];
			$array = Buscar($_SESSION['unidade_id']);
			$nome = $array['nome'];
			$abreviatura = $array['abreviatura'];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['unidade_id'] = $_GET['id'];
			Excluir($_SESSION['unidade_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		//Se for uma atualização de registro...
			...
			Editar($_SESSION['unidade_id'],$nome,$abreviatura);
		...
		<div class="conteudo">
          ...
          <li class="menu_destacado_borda_esquerda">
			...
			<div class="principal">
				...
				<form name="form_unidade" method="post" enctype="multipart/form-data" action="cadastro_unidade.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						...
						<table>
							...
							<tr>
								<td>Abreviatura:</td>
								<td><input required class="cadastro_input" type="text" name="abreviatura" placeholder="Abreviatura" value="<?php echo $abreviatura ?>"></td>
							</tr>
							<tr>
								<td></td>
								<td></td>
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização de Unidades</h3>
					</div>
					<table>
