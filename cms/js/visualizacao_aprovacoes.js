...
$(document).ready(function(){
	carregarAprovacoes();
	carregarAprovadosAdministrativo();
});
...
function carregarAprovadosAdministrativo(){
	$.ajax({
		url:"json/aprovacoes.php?listar_pratos_aprovados_administrativo=0",
		dataType:"json",
		success:function(data){
			...
			for(var i=0; i<data.length; i++){
					...
					var aprovacao
					if(data[i].aprovacao_id == 1){
						aprovacao = 'aguardando_aprovacao';
					}else if(data[i].aprovacao_id == 2){
						aprovacao = 'aprovado';
					}else if(data[i].aprovacao_id == 3){
