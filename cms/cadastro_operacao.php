<?php
	...
	require_once('php/crud_operacao.php');
  ...
  if(!isset($_SESSION)) {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['operacao_id'] = $_GET['id'];
			$array = Buscar($_SESSION['operacao_id']);
			$nome = $array['nome'];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['operacao_id'] = $_GET['id'];
			Excluir($_SESSION['operacao_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		//Se for uma atualização de registro...
			...
			Editar($_SESSION['operacao_id'],$nome);
		...
		<div class="conteudo">
          ...
          <li class="menu_destacado_borda_esquerda">
			...
			<div class="principal">
				...
				<form name="form_operacao" method="post" enctype="multipart/form-data" action="cadastro_operacao.php<?php echo $modo ?>">
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização de Operação</h3>
					</div>
					<table>
