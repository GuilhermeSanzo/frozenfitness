<?php
	...
	require_once('php/crud_artigo.php');
  ...
  if(!isset($_SESSION)) {
	...
	/* Se ela não estiver... */
		...
		AutenticaMarketing($tipo_usuario_id);
	...
	$artigo_data = "";
	$autor = "";
	$categoria_artigo="Categoria do Artigo";
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['artigo_id'] = $_GET['id'];
			$array = Buscar($_SESSION['artigo_id']);
			$titulo = $array['titulo'];
			$conteudo = $array['conteudo'];
			$artigo_data = $array['artigo_data'];
			$autor = $array['autor'];
			$categoria_artigo = $array['categoria_artigo'];
			$value_categoria_artigo = $array["categoria_artigo_id"];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['artigo_id'] = $_GET['id'];
			Excluir($_SESSION['artigo_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		$artigo_data = $_POST['artigo_data'];
		$autor = $_POST['autor'];
		$value_categoria_artigo = $_POST["categoria_artigo_id"];

		if (strpos($artigo_data, '/') !== false) {
		    $artigo_data = date('Y-m-d', strtotime($artigo_data));
		}
		...
		if($modo == 'novo'){
			Inserir($titulo,$conteudo,$artigo_data,$autor,$value_categoria_artigo,$caminho_imagem,$imagem);
		//Se for uma atualização de registro...
		}else if($modo == 'atualizar'){
			Editar($_SESSION['artigo_id'],$titulo,$conteudo,$artigo_data,$autor,$value_categoria_artigo,$caminho_imagem,$imagem);
...
<head>
	...
	<script src="js/cadastro_artigo.js"> </script>
	...
	<div class="corpo">
		...
		<div class="conteudo">
			...
			<div class="principal">
				...
				<form name="form_ingrediente" method="post" enctype="multipart/form-data" action="cadastro_artigo.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						...
						<table>
							<tr>
								<td>Título: </td>
								<td><input required maxlength="145" class="cadastro_input" type="text" name="titulo" placeholder="Nome" value="<?php echo $titulo ?>"></td>
							</tr>
							<tr>
								<td>Conteúdo:</td>
								<td><textarea required class="cadastro_input" style="height:150px;border-radius:10px" type="text" name="conteudo" placeholder="Conteúdo"><?php echo $conteudo ?></textarea></td>
							</tr>
							<tr>
								<td>Selecione uma imagem: </td>
								<td><input required  name="arquivo" type="file" id="filer_input" multiple="multiple" data-jfiler-limit="1"/></td>
							</tr>
							<tr>
								<td>Data do artigo</td>
								<td><input required id="artigo_data" class="cadastro_input" type="date" name="artigo_data" placeholder="Data do artigo" value="<?php echo $artigo_data ?>"></td>
							</tr>
							<tr>
								<td>Autor</td>
								<td><input required class="cadastro_input" type="text" name="autor" placeholder="Autor" value="<?php echo $autor ?>"></td>
							</tr>

							<tr>
								<td>Categoria do Artigo: </td>
								<td>
									<select required class="cadastro_input" type="text" name="categoria_artigo_id">
										"<option value='<?php echo $value_categoria_artigo ?>'><?php echo $categoria_artigo ?></option>"
										<?php
											$conexao = connect();
											$sql = "select * from categoria_artigo";
											$select = mysqli_query($conexao,$sql);
											while($array = mysqli_fetch_array($select)){
												echo "<option value=".$array['categoria_artigo_id'].">".$array['nome']."</option>";
											}
										?>
										<optgroup>
