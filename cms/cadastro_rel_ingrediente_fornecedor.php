<?php
	...
	require_once('php/crud_rel_ingrediente_fornecedor.php');
  ...
  if(!isset($_SESSION)) {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['rel_ingrediente_fornecedor_id'] = $_GET['id'];
			$array = Buscar($_SESSION['rel_ingrediente_fornecedor_id']);

			$ingrediente = $array['ingrediente'];
			$fornecedor = $array['fornecedor'];

			$value_ingrediente = $array["ingrediente_id"];
			$value_fornecedor = $array["fornecedor_id"];

			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['rel_ingrediente_fornecedor_id'] = $_GET['id'];
			Excluir($_SESSION['rel_ingrediente_fornecedor_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		if($modo == 'novo'){
			Inserir($ingrediente,$fornecedor);

		//Se for uma atualização de registro...
		}else if($modo == 'atualizar'){
			Editar($_SESSION['rel_ingrediente_fornecedor_id'],$ingrediente,$fornecedor);
...
<head>
	...
	<title>Cadastro do Relacionamento entre Ingredientes e Fornecedores</title>
		...
		<div class="conteudo">
          ...
          <li class="menu_destacado_borda_esquerda">
			...
			<div class="principal">
				...
				<form name="form_rel_ingrediente_fornecedor" method="post" enctype="multipart/form-data" action="cadastro_rel_ingrediente_fornecedor.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						<div class="cadastro_ingrediente_titulo">
							<h3>Cadastro de Relacionamento entre Ingrediente e Fornecedor</h3>
						</div>
						...
						<table>
							<tr>
								<td>Ingrediente: </td>
								<td>
									<select required  class="selecao_cadastro" class="cadastro_input" type="text" name="ingrediente">
										"<option value='<?php echo $value_ingrediente ?>'><?php echo $ingrediente ?></option>"
										<?php
											$conexao = connect();
											$sql = "select * from ingrediente";
											$select = mysqli_query($conexao,$sql);
											while($array = mysqli_fetch_array($select)){
												echo "<option value=".$array['ingrediente_id'].">".$array['nome']."</option>";
											}
										?>
										<optgroup>
							...
							<tr>
								<td>Fornecedor: </td>
								<td>
									<select required class="selecao_cadastro" class="cadastro_input" type="text" name="fornecedor">
										"<option value='<?php echo $value_fornecedor?>'><?php echo $fornecedor ?></option>"
										<?php
											$conexao = connect();
											$sql = "select * from fornecedor";
											$select = mysqli_query($conexao,$sql);
											while($array = mysqli_fetch_array($select)){
												echo "<option value=".$array['fornecedor_id'].">".$array['nome_fantasia']."</option>";
											}
										?>
										<optgroup>
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização do Relacionamento entre Ingredientes e Fornecedores</h3>
					</div>
					<table>
