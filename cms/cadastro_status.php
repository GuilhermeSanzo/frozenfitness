<?php
	...
	require_once('php/crud_status.php');
  ...
  if(!isset($_SESSION)) {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			...
			$array = Buscar($_SESSION['status_id']);
			$nome = $array['nome'];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['status_id'] = $_GET['id'];
			Excluir($_SESSION['status_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		//Se for uma atualização de registro...
			...
			Editar($_SESSION['status_id'],$nome);
		...
		<div class="conteudo">
          ...
          <li class="menu_destacado_borda_esquerda">
			...
			<div class="principal">
				...
				<!-- Cadastro de Status -->
				<form name="form_status" method="post" enctype="multipart/form-data" action="cadastro_status.php<?php echo $modo ?>">
