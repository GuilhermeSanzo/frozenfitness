...
<?php
	...
	require_once('php/crud_promocao.php');
	...
	if (empty($_SESSION["nome"])) {
		echo "<script> alert('Você está tentando acessar uma página restrita!')</script>";
	/* Se ela não estiver... */
	} else {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			...
			$_SESSION['promocao_id'] = $_GET['id'];
			$array = Buscar($_SESSION['promocao_id']);
	...
	if(isset($_POST['submit'])){
		...
		if($modo == 'novo'){
			Inserir($nome,$desconto);

		//Se for uma atualização de registro...
		}else if($modo == 'atualizar'){
			Editar($_SESSION['promocao_id'],$nome,$desconto);
...
<head>
	...
	<script src="js/cadastro_promocao.js"> </script>
	...
	<div class="corpo">
		...
		<div class="conteudo">
			...
			<div class="principal">
				...
				<!-- Cadastro de Banners -->
				<form name="form_promocao" method="post" enctype="multipart/form-data" action="cadastro_promocao.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						...
						<table>
							...
							<tr>
								<td>Desconto (em Porcentagem): </td>
								<td><input required id="txtDesconto" class="cadastro_input" type="text" name="desconto" placeholder="Ex: 20.05 equivale a 20,05%" value="<?php echo $desconto ?>"></td>
							</tr>
							<tr>
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização de Promoção</h3>
					</div>
					<table>
