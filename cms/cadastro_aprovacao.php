<?php
	...
	require_once('php/crud_aprovacao.php');
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['aprovacao_id'] = $_GET['id'];
			$array = Buscar($_SESSION['aprovacao_id']);
			$display = "";
			$nome = $array['nome'];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['aprovacao_id'] = $_GET['id'];
			Excluir($_SESSION['aprovacao_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		if($modo == 'novo'){
			Inserir($nome,$abreviatura);

		//Se for uma atualização de registro...
		}else if($modo == 'atualizar'){
			Editar($_SESSION['aprovacao_id'],$nome,$abreviatura);
			$display="style='display:none'";
		...
		<div class="conteudo">
          ...
          <li class="menu_destacado_borda_esquerda">
			...
			<div class="principal">
				...
				<form name="form_aprovacao" method="post" enctype="multipart/form-data" action="cadastro_aprovacao.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						<div class="cadastro_ingrediente_titulo">
							<h3>Cadastro de aprovações</h3>
						</div>
						...
						<table>
							...
							<tr>
								<td><input <?php echo $display ?>class="botoes_cadastro" type="submit" name="submit" value="<?php echo $botao ?>"></td>
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização de aprovacaos</h3>
					</div>
					<table>
