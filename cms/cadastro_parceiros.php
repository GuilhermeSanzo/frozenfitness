<?php
	...
	require_once('php/crud_parceiro.php');
  ...
  if(!isset($_SESSION)) {
	...
	if(isset($_GET['modo'])){
		...
		if($modo == 'editar'){
			$modo = '?modo=atualizar';
			$_SESSION['parceiro_id'] = $_GET['id'];
			$array = Buscar($_SESSION['parceiro_id']);
			$nome_parceiro = $array['parceiro'];
			$telefone_parceiro = $array['telefone'];
			$caminho_imagem = $array['caminho_imagem'];
			$estado_parceiro = $array['estado'];
			$cidade_parceiro = $array['cidade'];
			$bairro_parceiro = $array['bairro'];
			$logradouro_parceiro = $array['nome_rua'];
			$numero_parceiro = $array['numero'];
			$cep_parceiro = $array['cep'];
			$botao = 'Editar';
		}if($modo == 'excluir'){
			$_SESSION['parceiro_id'] = $_GET['id'];
			Excluir($_SESSION['parceiro_id']);
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		if($modo == 'novo'){
			Inserir($nome,$caminho_imagem,$imagem,$telefone,$logradouro,$numero,$bairro,$cep,$cidade,$estado);

		//Se for uma atualização de registro...
		}else if($modo == 'atualizar'){
			Editar($_SESSION['parceiro_id'],$nome,$caminho_imagem,$imagem,$telefone,$logradouro,$numero,$bairro,$cep,$cidade,$estado);
...
<head>
	...
	<script src="js/cadastro_parceiros.js"> </script>
	...
	<div class="corpo">
		...
		<div class="conteudo">
			...
			<div class="principal">
				...
				<form name="form_parceiro" method="post" enctype="multipart/form-data" action="cadastro_parceiros.php<?php echo $modo ?>">
					<div class="cadastro_ingrediente">
						...
						<table>
							<tr>
								<td>Nome: </td>
								<td><input required class="cadastro_input" type="text" name="nome" placeholder="Nome" value="<?php echo $nome_parceiro ?>"></td>
							</tr>
							<tr>
							...
							<tr>
								<td>Telefone: </td>
								<td><input id="txtTelefone" required class="cadastro_input" type="text" name="telefone" placeholder="Telefone" value="<?php echo $telefone_parceiro ?>"></td>
							</tr>
							<tr>
								<td>Rua: </td>
								<td><input required class="cadastro_input" type="text" name="logradouro" placeholder="Nome da Rua" value="<?php echo $logradouro_parceiro ?>"></td>
							</tr>
							<tr>
								<td>Número: </td>
								<td><input required class="cadastro_input" type="text" name="numero" placeholder="Número" value="<?php echo $numero_parceiro ?>"></td>
							</tr>
							<tr>
								<td>Bairro: </td>
								<td><input required class="cadastro_input" type="text" name="bairro" placeholder="Bairro" value="<?php echo $bairro_parceiro ?>"></td>
							</tr>
							<tr>
								<td>CEP: </td>
								<td><input id="txtCep" required class="cadastro_input" type="text" name="cep" placeholder="CEP" value="<?php echo $cep_parceiro ?>"></td>
							</tr>
							<tr>
								<td>Cidade: </td>
								<td><input required class="cadastro_input" type="text" name="cidade" placeholder="Cidade" value="<?php echo $cidade_parceiro ?>"></td>
							</tr>
							<tr>
								<td>Estado: </td>
								<td><input required class="cadastro_input" type="text" name="estado" placeholder="Estado" value="<?php echo $estado_parceiro ?>"></td>
							</tr>
							<tr>
				...
				<div class="visualizacao_ingrediente">
					<div class="cadastro_ingrediente_titulo">
						<h3>Visualização de Parceiros</h3>
					</div>
					<table>
