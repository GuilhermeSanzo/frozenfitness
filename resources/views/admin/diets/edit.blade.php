    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-3xl border border-gray-100 p-8 lg:p-12">
                <form action="{{ route('diets.update', $diet) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-8">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $diet->name) }}" required
                            ...
                            <div>
                                ...
                                <select name="category_id" id="category_id" required
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_id', $diet->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" required
                                    class="w-full bg-gray-50 border-gray-100 rounded-2xl px-6 py-4 focus:ring-green-500 focus:border-green-500 transition font-bold text-gray-900 shadow-sm"
                                    placeholder="Describe the diet goals and schedule...">{{ old('description', $diet->description) }}</textarea>
                                @error('description')
                                    <p class="mt-2 text-sm text-red-600 font-bold">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex items-center">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_approved" value="1" class="sr-only peer" {{ old('is_approved', $diet->is_approved) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 rounded-3xl p-6 max-h-[500px] overflow-y-auto border border-gray-100 space-y-4">
                                @php
                                    $dietMeals = $diet->meals->pluck('pivot.day', 'id')->toArray();
                                @endphp
                                @foreach($meals as $meal)
                                    <div class="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-50 shadow-sm group hover:border-blue-200 transition">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <input type="number" name="meals[{{ $meal->id }}]" 
                                                value="{{ old('meals.' . $meal->id, $dietMeals[$meal->id] ?? '') }}"
                                                class="w-16 bg-gray-50 border-gray-100 rounded-xl px-2 py-2 text-center font-bold text-gray-900 focus:ring-blue-500 focus:border-blue-500 transition"
                                                placeholder="-" min="0">
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div class="flex items-center justify-end gap-6 pt-10 mt-10 border-t border-gray-50">
                        ...
                        <button type="submit" class="bg-blue-600 text-white px-12 py-4 rounded-2xl font-bold text-lg hover:bg-blue-700 transition shadow-lg hover:shadow-blue-200 active:scale-95 duration-200">
                            Update Diet Plan
                        </button>
                    </div>
                </form>
