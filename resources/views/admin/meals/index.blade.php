<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('meals.create') }}" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition font-bold text-sm">
                + New Meal
            </a>
        </div>
    </x-slot>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                ...
                                <th class="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest">Price</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($meals as $meal)
                                <tr class="hover:bg-gray-50/50 transition">
                                    ...
                                    <td class="px-8 py-5">
                                        <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-white" style="background-color: {{ $meal->category->color }}">
                                            {{ $meal->category->name }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-5 font-mono text-gray-600">
                                        ${{ number_format($meal->unit_price, 2) }}
                                    </td>
                                    <td class="px-8 py-5">
                                        @if($meal->is_approved)
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('meals.edit', $meal) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition">
