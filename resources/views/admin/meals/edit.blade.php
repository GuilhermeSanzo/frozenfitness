                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                ...
                                <input type="number" name="unit_price" id="unit_price" value="{{ old('unit_price', $meal->unit_price) }}" required step="0.01" min="0"
                                    class="w-full bg-gray-50 border-gray-100 rounded-2xl px-6 py-4 focus:ring-green-500 focus:border-green-500 transition font-bold text-gray-900 shadow-sm"
                                    placeholder="0.00">
                                @error('unit_price')
                                    <p class="mt-2 text-sm text-red-600 font-bold">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" required
                                    class="w-full bg-gray-50 border-gray-100 rounded-2xl px-6 py-4 focus:ring-green-500 focus:border-green-500 transition font-bold text-gray-900 shadow-sm"
                                    placeholder="Describe the meal ingredients and benefits...">{{ old('description', $meal->description) }}</textarea>
                                @error('description')
                                    <p class="mt-2 text-sm text-red-600 font-bold">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex items-center">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_approved" value="1" class="sr-only peer" {{ old('is_approved', $meal->is_approved) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 rounded-3xl p-6 max-h-[500px] overflow-y-auto border border-gray-100 space-y-4">
                                @php
                                    $mealIngredients = $meal->ingredients->pluck('pivot.quantity_grams', 'id')->toArray();
                                @endphp
                                    ...
                                    <div class="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-50 shadow-sm group hover:border-blue-200 transition">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <input type="number" name="ingredients[{{ $ingredient->id }}]" 
                                                value="{{ old('ingredients.' . $ingredient->id, $mealIngredients[$ingredient->id] ?? '') }}"
                                                class="w-24 bg-gray-50 border-gray-100 rounded-xl px-3 py-2 text-center font-bold text-gray-900 focus:ring-blue-500 focus:border-blue-500 transition"
                                                placeholder="0" min="0">
