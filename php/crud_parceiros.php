<?php 
	...
	function Listar() {
		$conexao = connect();
	    $sql = "select p.parceiro_id,p.nome as parceiro,p.caminho_imagem,p.telefone,e.nome as 'nome_rua' ,e.nome, e.numero,e.cep,e.bairro,e.cidade,e.estado,p.link
				from parceiro as p 
	    ...
	    while ($array = mysqli_fetch_array($select)) {
	    	echo'
				...
				<div class="parceiros_descricao">
					<p>
						Rua '.$array["nome_rua"].', '.$array["numero"].' - '.$array["bairro"].' - '.$array["cidade"].' - '.$array["estado"].' - '.$array["cep"].'
					</p>
				</div>
