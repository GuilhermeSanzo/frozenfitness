<?php 
	...
	function ListarBarraBusca($txtBarraBusca) {
		$conexao = connect();
		...
		$sql = "(select 
				...
				where p.nome like '%".$txtBarraBusca."%' 
				...
				where d.nome like '%".$txtBarraBusca."%' 
				and d.aprovacao_id = 2 
				and p.aprovacao_id = 2 
				group by dieta_id);";
	   ...
	   //  $sql = "select 
	    ...
	    if(mysqli_num_rows($select) == 0){
	    	echo '<div class="nada_encontrado">Não foi encontrado nenhum prato ou dieta para "'.$txtBarraBusca.'"</div>';
	    }else{
	    ...
	    while ($array = mysqli_fetch_array($select)) {
	    	...
	    	if($array['tipo'] == "prato"){
	    		echo '<div class="box_produto">'.
									...
									'</div>'.
									'<a class="box_item_menu_estilo" href="php/add_carrinho.php?action=insert&prato_id='.$array["prato_id"].'">'.
	    		...
	    		echo '<div class="box_produto">'.
									...
									'<div class="preco_kcal">'.
										...
										'<div style="clear:both"></div>'.
									'</div>'.
									'<a class="box_item_menu_estilo" href="php/add_carrinho.php?action=insert&dieta_id='.$array["prato_id"].'">'.
										'<div class="box_botao_produto" id="comprar">'.
											'Comprar'.
										'</div>'.
									'</a>'.
