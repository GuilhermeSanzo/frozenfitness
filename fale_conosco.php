<?php
	...
	if(isset($_POST['submit'])){
		...
		$titulo = ucfirst(strtolower($_POST['txt_titulo']));
		$mensagem = ucfirst($_POST['txt_mensagem']);
		$categoria = $_POST['txt_categoria'];

		//Se for um novo registro...
		if($modo == 'novo'){
			if (!empty($nome) && !empty($email) && !empty($titulo) && !empty($mensagem) && !empty($categoria)) {
				Inserir($nome,$email,$titulo,$mensagem,$categoria);
			}
...
<head>
	...
	<title>Fale Conosco - Frozen Fitness Gourmet</title>
	<link rel="icon" type="image/gif" href="imagens/logo.png">
	
	<!-- Estilização Desktop -->
	<link href="css/template.css" rel="stylesheet"/>
	<link href="css/fale_conosco.css" rel="stylesheet"/>

	<!-- Estilização Desktop -->
	<link href="css/mobile/template.css" rel="stylesheet"/>
	<link href="css/mobile/fale_conosco.css" rel="stylesheet"/>
		...
		$(document).ready(function() {
			...
			if(window.location.href.indexOf("modo=novo") > -1) {
				bootbox.alert({
					...
					size: 'small',
					callback: function(){
							$(location).attr('href','fale_conosco.php');
					}
				});
			}
    	});
...
<body>
	...
	<div class="esp_conteudo">
		<div class="conteudo">
			<form name="form_fale_conosco" id="form_fale_conosco" method="post" action="fale_conosco.php<?php echo $modo ?>">
				<div class="box_form_fale_conosco">
					<div class="caixa_texto">
						<div class="title_caixa_texto">Nome</div>
						<div class="input_caixa_texto">
							<input type="text" class="input_text" placeholder="Insira seu nome" name="txt_nome" id="txt_nome" required>
						</div>
					</div>
					<div class="caixa_texto">
						<div class="title_caixa_texto">E-mail</div>
						<div class="input_caixa_texto">
							<input type="email" class="input_text" placeholder="Insira seu e-mail" name="txt_email" id="txt_email" required>
						</div>
					</div>
					<div class="caixa_spinner">
						<label>
						    <select name="txt_categoria" id="txt_categoria" required>
						        ...
						        <option value="<?php echo $array['categoria_id'] ?>"> <?php echo $array['nome'] ?> </option>
					...
					<div class="caixa_texto">
						<div class="title_caixa_texto">Título</div>
						<div class="input_caixa_texto">
							<input type="text" class="input_text" placeholder="Insira o título da mensagem" name="txt_titulo" id="txt_titulo" required>
						</div>
					</div>
					<div class="caixa_texto">
						<div class="title_caixa_texto">Mensagem</div>
						<div class="input_caixa_texto">
							<textarea class="input_text_area" placeholder="Informe sua mensagem aqui" name="txt_mensagem" id="txt_mensagem" required></textarea>
						</div>
					</div>
					<div class="caixa_texto">
						<div class="input_caixa_texto">
							<input type="submit" id="submit" name="submit" class="form_submit" value="Enviar">
						</div>
					</div>
				</div>
			</form>
